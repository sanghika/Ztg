...
// FIX #11: Was 5 minutes (300 s). Reduced to 2 minutes — a real OAuth flow
// completes in seconds; 5 minutes leaves the button disabled far too long if
// the user simply closes the popup.
 ...
 *
 * FIX #12: Align the client-side isTrustedOrigin with the server-side version:
 * check protocol explicitly so a non-http(s) URL whose hostname happens to be
...
export default function App() {
  ...
  // FIX #13: fetchRepos needs its own AbortController so that if the token
  ...
  // FIX #11: Keep a ref to the connect-timeout timer so it can be cleared on
  ...
  useEffect(() => {
    return () => {
      ...
      // FIX #11 applied: clear the connect timeout on unmount.
  ...
  // ── FIX #14: Prevent the browser from opening a dropped file as a new page.
  ...
  useEffect(() => {
    const preventBrowserDrop = (e: DragEvent) => {
      // Allow only if the event target is inside our app root
      e.preventDefault();
    };
  ...
  useEffect(() => {
    const handleMessage = (event: MessageEvent) => {
      ...
      // FIX #12 applied: use the tightened isTrustedOrigin.
      if (
      ...
      if (event.data?.type === "OAUTH_AUTH_SUCCESS") {
        ...
        if (typeof t === "string" && t.length > 0) {
          // FIX #11 applied: clear the safety timer when auth succeeds so it
          // doesn't fire redundantly after the button is already re-enabled.
          if (connectTimerRef.current !== null) {
            clearTimeout(connectTimerRef.current);
  ...
  const fetchRepos = useCallback(async (currentToken: string) => {
    // FIX #13 applied: cancel any in-flight repo fetch before starting a new one.
    fetchAbortRef.current?.abort();
    ...
    try {
      ...
      // FIX #13 applied: AbortError means a newer fetch superseded this one —
      // silently discard rather than showing an error to the user.
  ...
  const handleConnect = async () => {
    ...
    try {
      ...
      const popup = window.open(url, "oauth_popup", "width=600,height=700,noopener=no");
      if (!popup) {
        // FIX #15: When popup is blocked, the original code dispatched
        // SET_CONNECTING(false) but the safety setTimeout was set AFTER this
        // block and would still run, leaving a dangling timer.  Restructured
        // so the timer is only created when the popup successfully opens.
      ...
      // FIX #11 applied: store the timer ref so it can be cancelled.
